...
PC 管理台的 **RS485 传感器** 页面顶部可以选择门限模式：

- **单门限**：可选择 RS485-1 或 RS485-2 作为关门到位传感器。检测到门限表示已关门；长时间未检测到门限时，继续沿用原有自动关门逻辑。默认选择 RS485-2，以兼容旧版本配置。未选择的另一路不自动轮询、不恢复传感器配置，也不产生离线告警。
- **上下门限**：端口用途固定，RS485-1 是上门限（车库门全开），RS485-2 是下门限（车库门全关），不可互换。

上下门限模式可以区分全开、全关、运动中、中途停止、行程超时、传感器冲突和读取失败。两路门限均未触发时，系统结合传感器运行/震动信号和最近到达的端点推断开门或关门方向。超过可配置的行程超时时间仍未到达任一门限时，状态变为 `TIMEOUT` 并发送告警。

为避免单按键“开 / 停 / 关”控制器被错误切换方向，上下门限模式只有在确认到达上门限（全开）后才会自动发送关门指令；运动中、中途停止、超时、冲突或未知状态都只复查和告警，不会盲目触发电机。两只门限传感器同时触发会被判定为 `CONFLICT`。
...
| GET | `/api/rs485/door-config` | 读取单门限/上下门限配置 |
| POST | `/api/rs485/door-config` | 保存门限模式、单门限端口和行程超时时间 |
...
- S3 每秒更新一次门状态，并通过飞书发送门打开、门关闭和长时间未关闭通知。门打开通知会标明人脸、掌静脉、Web/API、自动关门异常或手动/外部按钮等来源；长时间未关闭默认阈值为 300 秒，每次开门周期只提醒一次。
- 所有飞书消息统一包含精确到秒的北京时间；NTP 尚未同步时会明确显示“时间未同步”和设备运行秒数，不使用伪造的系统时间。
